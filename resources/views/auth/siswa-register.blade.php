...
<div class="min-h-screen w-full flex flex-col md:flex-row bg-[#0A2342] relative overflow-hidden font-sans">
    ...
    <div class="hidden md:flex md:w-[52%] lg:w-[56%] min-h-screen bg-gradient-to-br from-[#0B2A4C] via-[#0A2342] to-[#071930] flex-col justify-center px-12 lg:px-20 text-white relative">
        <div class="max-w-xl relative z-10">
            <div class="flex items-center gap-3 mb-4">
                <span class="text-xs font-semibold uppercase tracking-widest text-slate-300">REGISTRASI SISWA</span>
            </div>
    ...
    <div class="relative w-full md:w-[48%] lg:w-[44%] min-h-screen flex items-center z-10">
        ...
        <!-- Primary White Card -->
        <div class="w-full h-full min-h-screen bg-white rounded-l-none md:rounded-l-[3.2rem] shadow-2xl px-6 sm:px-12 lg:px-16 py-10 flex flex-col justify-between relative z-10 my-auto">

            <!-- Main Form Section -->
            <div class="my-auto py-6">
                <!-- Title -->
                <div class="text-center mb-6">
                    <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Daftar Akun Siswa</h2>
                    <p class="text-xs text-slate-500 mt-1">Buat akun siswa baru di e-Learning TIK</p>
                </div>

                <!-- Role Selector Buttons -->
                <div class="p-1 bg-slate-100 rounded-2xl flex gap-1 mb-5 border border-slate-200/80">
                    <a href="{{ route('siswa.register') }}" class="flex-1 py-2 px-3 text-xs font-semibold rounded-xl text-center transition bg-white text-[#3B5284] shadow-sm border border-slate-200">
                        Siswa (NISN)
                    </a>
                    <a href="{{ route('admin.register') }}" class="flex-1 py-2 px-3 text-xs font-semibold rounded-xl text-center transition text-slate-600 hover:text-slate-900">
                        Guru / Pendidik
                    </a>
                </div>
                ...
                @if ($errors->any())
                    <div class="mb-4 p-3 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 text-xs space-y-1">
                ...
                <!-- Form -->
                <form action="{{ route('siswa.register.attempt') }}" method="POST" class="space-y-3">
                    @csrf
                    <div>
                        <label class="block text-xs font-medium text-slate-600 mb-1">NISN (10 Digit)</label>
                        <input type="text" name="nisn" value="{{ old('nisn') }}" maxlength="10" placeholder="Masukkan 10 digit NISN Anda"
                            class="w-full px-3.5 py-2.5 text-xs sm:text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-[#3B5284] focus:border-[#3B5284] outline-none transition text-slate-800 placeholder-slate-400" required>
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-slate-600 mb-1">Nama Lengkap</label>
                        <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap') }}" placeholder="Contoh: Budi Santoso"
                            class="w-full px-3.5 py-2.5 text-xs sm:text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-[#3B5284] focus:border-[#3B5284] outline-none transition text-slate-800 placeholder-slate-400" required>
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-slate-600 mb-1">Email Sekolah / Pribadi</label>
                        <input type="email" name="email" value="{{ old('email') }}" placeholder="siswa@smkn2jember.sch.id"
                            class="w-full px-3.5 py-2.5 text-xs sm:text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-[#3B5284] focus:border-[#3B5284] outline-none transition text-slate-800 placeholder-slate-400" required>
                    </div>
                ...
                <div class="mt-5 text-center text-xs text-slate-500">
                    Sudah memiliki akun terdaftar? <a href="{{ route('siswa.login') }}" class="text-[#3B5284] hover:underline font-bold">Login Sekarang</a>
                </div>
            </div>
