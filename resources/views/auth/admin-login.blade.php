    ...
    <div class="hidden md:flex md:w-[52%] lg:w-[56%] min-h-screen bg-gradient-to-br from-[#0B2A4C] via-[#0A2342] to-[#071930] flex-col justify-center px-12 lg:px-20 text-white relative">
        <div class="max-w-xl relative z-10">
            <div class="flex items-center gap-3 mb-4">
                <span class="text-xs font-semibold uppercase tracking-widest text-slate-300">PORTAL GURU</span>
            </div>
        ...
        <div class="w-full h-full min-h-screen bg-white rounded-l-none md:rounded-l-[3.2rem] shadow-2xl px-6 sm:px-12 lg:px-16 py-12 flex flex-col justify-between relative z-10">
            ...
            <div class="my-auto py-8">
                ...
                <div class="text-center mb-8">
                    <h2 class="text-3xl font-bold text-slate-800 tracking-tight">Login Guru</h2>
                    <p class="text-xs text-slate-500 mt-1">Masuk ke Portal Pendidik &amp; Administrasi</p>
                </div>

                <!-- Role Selector Buttons -->
                <div class="p-1 bg-slate-100 rounded-2xl flex gap-1 mb-6 border border-slate-200/80">
                    <a href="{{ route('siswa.login') }}" class="flex-1 py-2.5 px-3 text-xs font-semibold rounded-xl text-center transition text-slate-600 hover:text-slate-900">
                        Siswa (NISN)
                    </a>
                    <a href="{{ route('admin.login') }}" class="flex-1 py-2.5 px-3 text-xs font-semibold rounded-xl text-center transition bg-white text-[#3B5284] shadow-sm border border-slate-200">
                        Guru / Pendidik
                    </a>
                </div>
                ...
                <!-- Alerts -->
                @if (session('success'))
                    <div class="mb-4 p-3.5 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-medium">
                        {{ session('success') }}
                    </div>
                @endif

                @if ($errors->any())
                    <div class="mb-4 p-3.5 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 text-xs space-y-1">
                ...
                <form action="{{ route('admin.login.attempt') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-xs font-medium text-slate-600 mb-1.5">Nomor Induk Pegawai (NIP)</label>
                        <input type="text" name="nip" value="{{ old('nip') }}" maxlength="18" placeholder="Masukkan 18 digit NIP Anda"
                            class="w-full px-4 py-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-[#3B5284] focus:border-[#3B5284] outline-none transition text-slate-800 placeholder-slate-400" required>
                    </div>
