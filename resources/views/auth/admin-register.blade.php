...
<div class="min-h-screen w-full flex flex-col md:flex-row bg-[#0A2342] relative overflow-hidden font-sans">
    ...
    <div class="relative w-full md:w-[48%] lg:w-[44%] min-h-screen flex items-center z-10">
        ...
        <!-- Primary White Card -->
        <div class="w-full h-full min-h-screen bg-white shadow-2xl px-6 sm:px-12 lg:px-16 py-8 flex flex-col justify-between relative z-10 my-auto">
            ...
            <div class="my-auto py-4">
                ...
                <div class="p-1 bg-slate-100 flex gap-1 mb-4 border border-slate-200/80">
                    <a href="{{ route('siswa.register') }}" class="flex-1 py-2 px-3 text-xs font-semibold text-center transition text-slate-600 hover:text-slate-900">
                        Siswa (NISN)
                    </a>
                    <a href="{{ route('admin.register') }}" class="flex-1 py-2 px-3 text-xs font-semibold text-center transition bg-white text-[#3B5284] shadow-sm border border-slate-200">
                        Guru / Pendidik
                    </a>
                </div>
                ...
                <!-- Alerts -->
                @if (session('success'))
                    <div class="mb-3 p-3 bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-medium">
                        {{ session('success') }}
                    </div>
                @endif

                @if ($errors->any())
                    <div class="mb-3 p-3 bg-rose-50 border border-rose-200 text-rose-700 text-xs space-y-1">
                ...
                <form action="{{ route('admin.register.attempt') }}" method="POST" enctype="multipart/form-data" class="space-y-2.5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        <div>
                            ...
                            <input type="password" name="password" placeholder="Min. 6 karakter"
                                class="w-full px-3 py-2 text-xs sm:text-sm bg-slate-50 border border-slate-200 focus:bg-white focus:ring-2 focus:ring-[#3B5284] focus:border-[#3B5284] outline-none transition text-slate-800 placeholder-slate-400" required>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <input type="file" name="foto_profile" accept="image/png,image/jpeg,image/jpg"
                            class="w-full text-xs text-slate-500 file:mr-3 file:py-1.5 file:px-3 file:border-0 file:text-xs file:font-semibold file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200 border border-slate-200 p-1 bg-slate-50">
                    </div>
