...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-1 bg-white rounded-lg shadow p-6 h-fit">
        ...
        <form action="{{ route('materi.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm font-medium mb-1">Diampu oleh (Admin/Guru)</label>
                <select name="nip"
                    ...
                    @foreach ($admins as $admin)
                        <option value="{{ $admin->nip }}" {{ old('nip') == $admin->nip ? 'selected' : '' }}>
                            {{ $admin->nama_lengkap }} ({{ $admin->nip }})
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="isi_materi" rows="4"
                          class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:ring focus:ring-blue-200 focus:outline-none">{{ old('isi_materi') }}</textarea>
                @error('isi_materi')<p class="text-xs text-red-600 mt-1">{{ $message }}</p>@enderror
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">Upload File (PDF/Word/PPT)</label>
                ...
                <tbody>
                    @forelse ($materis as $materi)
                        <tr class="border-b align-top">
                            <td class="px-3 py-2">
                                <p class="font-medium">{{ $materi->judul_materi }}</p>
                                <p class="text-gray-500 text-xs mt-1">{{ Str::limit($materi->isi_materi, 80) }}</p>
                            </td>
                            <td class="px-3 py-2">
                                {{ $materi->admin->nama_lengkap ?? '-' }}
                            </td>
                            <td class="px-3 py-2">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-3 py-6 text-center text-gray-400">Belum ada materi.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
